...
def bench_kineto(fn: Callable, num_tests: int = 30,
    ...
    schedule = paddle.profiler.make_scheduler(closed=0, ready=1, record=1, repeat=1) if not using_nsys else None
    profiler = paddle.profiler.Profiler(targets=[paddle.profiler.ProfilerTarget.CPU], scheduler=schedule) if not using_nsys else empty_suppress()
    with profiler:
        for i in range(2):
...
def parse_us(s: str) -> float:
    """
    Parse a string like '123 us' to float, return 123.0
    """
    return float(s.replace(" us", ""))

def strip_bracket_suffix(s: str) -> str:
    """
    Remove trailing '[...]' from a string.
    Example:
        'cudaSetDevice_v3020[0.935 us]' -> 'cudaSetDevice_v3020'
    """
    return s.split("[", 1)[0]

@dataclasses.dataclass
class BenchPaddleRawResult(BenchKinetoRawResult):
    """
    A struct holding the result of `bench_paddle`
    """
    pass

def bench_paddle(fn: Callable, num_tests: int = 30,
                 flush_l2: bool = True) -> BenchPaddleRawResult:
    """
    Run `fn` for `num_tests` times under `paddle.profiler.Profiler`, and returns a BenchPaddleRawResult
    """
    using_nsys = is_using_profiling_tools()
    ...
    def export_fn(prof):
        prof.export(path='./trace.json', format="json")

    schedule = paddle.profiler.make_scheduler(closed=0, ready=1, record=1, repeat=1) if not using_nsys else None
    profiler = paddle.profiler.Profiler(targets=[paddle.profiler.ProfilerTarget.GPU],
                                        scheduler=schedule, on_trace_ready=export_fn) if not using_nsys else empty_suppress()
    with profiler:
        for i in range(2):
            if i == 1 and not using_nsys:
                _run_profiler_range_start_marker_kernel()    # This marks the start of the profiling range
    ...
    if using_nsys:
        return BenchPaddleRawResult(True, num_tests, {})
    ...
    # Filter out all events before the range marker
    for idx, event in enumerate(events):
        if strip_bracket_suffix(event['name']) == "profiler_range_start_marker_kernel":
            events = events[idx+1:]
            break
    else:
        raise RuntimeError("Could not find profiler range start marker kernel event")
    ...
    for event in events:
        if 'cat' in event and event['cat'] == 'Kernel':
            kernel_name = strip_bracket_suffix(event['name'])
            if kernel_name not in kernel_times:
                kernel_times[kernel_name] = []
            kernel_times[kernel_name].append((parse_us(event['args']['start_time']) / 1e6, parse_us(event['args']['end_time']) / 1e6))

    return BenchPaddleRawResult(False, num_tests, kernel_times)