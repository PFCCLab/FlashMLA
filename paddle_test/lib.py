...
def _randperm_batch(batch_size: int, perm_range: torch.Tensor, perm_size: int, paddings: List[int]) -> torch.Tensor:
    ...
    rand[torch.arange(0, perm_range_max).broadcast_to(batch_size, perm_range_max) >= perm_range.view(batch_size, 1).to(torch.int64)] = float("-inf")    # Fill invalid positions, so that the following `topk` operators will select positions within `perm_range` first
...
class KVScope:
    ...
    def get_save_content(self):
        ...
        数据类型转换:
        - cache_seqlens: int32 -> int32 (直接兼容)
        - block_table: int32 -> int32 (直接兼容)
        - blocked_k: bfloat16 -> float32 (numpy 不支持 bf16)
        - abs_indices: int32 -> int32 (直接兼容)
        - indices_in_kvcache: int32 -> int32 (直接兼容)
        - topk_length: int32 -> int32 (Optional, 直接兼容)
        - blocked_k_quantized: float8_e4m3fn -> uint8 (numpy 不支持 fp8)
        """
        save_content = {
            "cache_seqlens": self.cache_seqlens.detach().cpu().numpy(),
            "block_table": self.block_table.detach().cpu().numpy(),
            "blocked_k": self.blocked_k.detach().cpu().float().numpy(),  # bf16 -> fp32
            "abs_indices": self.abs_indices.detach().cpu().numpy(),
            "indices_in_kvcache": self.indices_in_kvcache.detach().cpu().numpy(),
        }
        if self.topk_length is not None:
            save_content["topk_length"] = self.topk_length.detach().cpu().numpy()
        if self.blocked_k_quantized is not None:
            # fp8 -> uint8 (字节保存)
            save_content["blocked_k_quantized"] = self.blocked_k_quantized.detach().cpu().view(torch.uint8).numpy()
        return save_content

    @staticmethod
    def load_from_npz(data, t):
        """
        从 npz 数据加载 KVScope
        
        数据类型还原:
        - cache_seqlens: int32 -> torch.int32
        - block_table: int32 -> torch.int32
        - blocked_k: float32 -> torch.bfloat16
        - abs_indices: int32 -> torch.int32
        - indices_in_kvcache: int32 -> torch.int32
        - topk_length: int32 -> torch.int32 (Optional)
        - blocked_k_quantized: uint8 -> torch.float8_e4m3fn (Optional)
        """
        return KVScope(
            t,
            torch.tensor(data["cache_seqlens"], dtype=torch.int32),
            torch.tensor(data["block_table"], dtype=torch.int32),
            torch.tensor(data["blocked_k"], dtype=torch.float32).to(torch.bfloat16),  # fp32 -> bf16
            torch.tensor(data["abs_indices"], dtype=torch.int32),
            torch.tensor(data["indices_in_kvcache"], dtype=torch.int32),  # 注意: 是 int32 不是 bool!
            None if "topk_length" not in data else torch.tensor(data["topk_length"], dtype=torch.int32),
            None if "blocked_k_quantized" not in data else torch.tensor(data["blocked_k_quantized"], dtype=torch.uint8).view(torch.float8_e4m3fn),  # uint8 -> fp8
        )
...
class TestcaseForDecode:
    ...
    def dump_as_npz(self, path: str):
        """
        Save testcase for decode to npz file.

        Note: TestParam is not saved.
        """
        save_content = {
            "q": self.q.detach().cpu().float().numpy(),
            "sm_scale": self.sm_scale
        }
        if self.attn_sink is not None:
            save_content["attn_sink"] = self.attn_sink.detach().cpu().numpy()

        kv_scope_content = self.kv_scope.get_save_content()
        for k, v in kv_scope_content.items():
            save_content[f"kv_scope/{k}"] = v

        if self.extra_kv_scope is not None:
            extra_kv_scope_content = self.extra_kv_scope.get_save_content()
            for k, v in extra_kv_scope_content.items():
                save_content[f"extra_kv_scope/{k}"] = v

        np.savez(path, **save_content)

    @staticmethod
    def load_from_npz(path: str, p: TestParam):
        """
        从 npz 文件加载测试用例
        
        数据类型还原:
        - q: float32 -> torch.bfloat16
        - attn_sink: float32 -> torch.float32
        - sm_scale: float64 -> float
        """
        with np.load(path) as data:
            kv_scope_data = {k[len("kv_scope/"):]: v for k, v in data.items() if k.startswith("kv_scope/")}
            kv_scope = KVScope.load_from_npz(kv_scope_data, p)

            extra_kv_scope = None
            if "extra_kv_scope/cache_seqlens" in data:
                extra_kv_scope_data = {k[len("extra_kv_scope/"):]: v for k, v in data.items() if k.startswith("extra_kv_scope/")}
                extra_kv_scope = KVScope.load_from_npz(extra_kv_scope_data, p)

            return TestcaseForDecode(
                p,
                torch.tensor(data["q"], dtype=torch.float32).to(torch.bfloat16),  # fp32 -> bf16
                None if "attn_sink" not in data else torch.tensor(data["attn_sink"], dtype=torch.float32),
                float(data["sm_scale"]),
                kv_scope,
                extra_kv_scope
            )
...
def generate_testcase_for_decode(t: TestParam) -> TestcaseForDecode:
    ...
    def generate_one_k_scope(s_k: int, block_size: int, topk: int, is_varlen: bool, have_zero_seqlen: bool, is_all_indices_invalid: bool, have_topk_length: bool) -> KVScope:
        ...
        topk_length = torch.randint(0, topk+1, (b, ), dtype=torch.int32) if have_topk_length else None

        # Mask nonused KV as NaN
        if have_topk_length:
            indices_in_kvcache_masked = indices_in_kvcache.clone()
            indices_in_kvcache_masked[torch.arange(0, topk, dtype=torch.int32).view(1, 1, topk).broadcast_to([b, t.s_q, topk]) >= (topk_length.view(b, 1, 1) if have_topk_length else topk)] = -1
        else:
...
def count_flop_and_mem_vol(p: TestParam, t: Testcase) -> FlopsAndMemVolStatistics:
    ...
    if t.topk_length is not None:
        indices_valid_mask &= (torch.arange(p.topk, dtype=torch.int32)[None, None, :].broadcast_to(p.s_q, p.h_kv, p.topk)) < t.topk_length[:, None, None]
...
def count_flop_and_mem_vol_for_decode(p: TestParam, t: TestcaseForDecode) -> FlopsAndMemVolStatisticsForDecode:
    ...
    def get_num_retrieved_tokens(kv_scope: KVScope) -> int:
        if kv_scope.topk_length is None:
            ...
            mask = torch.arange(0, topk, dtype=torch.int32, device=indices.device).view(1, 1, topk).broadcast_to([batch, s_q, topk]) >= kv_scope.topk_length.view(batch, 1, 1)
            indices[mask] = -1
