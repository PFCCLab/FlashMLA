...
def gen_testcase() -> List[RawTestParam]:
    ...
    base_and_bszs = [
        ...
        # V3.2 Custom - topk=2048, q_token=1 (标准 decode)
        (RawTestParam(0, 128, 1, 1, 2 * 1024, True, topk=2048, d_qk=576), [1, 2, 3, 4, 5, 10, 20, 40, 60, 80,100, 128]),
        (RawTestParam(0, 128, 1, 1, 4 * 1024, True, topk=2048, d_qk=576), [1, 2, 3, 4, 5, 10, 20, 40, 60, 80,100, 128]),
        (RawTestParam(0, 128, 1, 1, 8 * 1024, True, topk=2048, d_qk=576), [1, 2, 3, 4, 5, 10, 20, 40, 60, 80,100, 128]),
        (RawTestParam(0, 128, 1, 1, 16 * 1024, True, topk=2048, d_qk=576), [1, 2, 3, 4, 5, 10, 20, 40, 60, 80,100, 128]),
        (RawTestParam(0, 128, 1, 1, 32 * 1024, True, topk=2048, d_qk=576), [1, 2, 3, 4, 5, 10, 20, 40, 60, 80,100, 128]),
        (RawTestParam(0, 128, 1, 1, 64 * 1024, True, topk=2048, d_qk=576), [1, 2, 3, 4, 5, 10, 20, 40, 60, 80,100]),
        (RawTestParam(0, 128, 1, 1, 128 * 1024, True, topk=2048, d_qk=576), [1, 2, 3, 4, 5, 10, 20, 40]),
        # V3.2 Custom - topk=4096, q_token=1 (标准 decode)
        (RawTestParam(0, 128, 1, 1, 2 * 1024, True, topk=4096, d_qk=576), [1, 2, 3, 4, 5, 10, 20, 40, 60, 80,100, 128]),
        (RawTestParam(0, 128, 1, 1, 4 * 1024, True, topk=4096, d_qk=576), [1, 2, 3, 4, 5, 10, 20, 40, 60, 80,100, 128]),
        (RawTestParam(0, 128, 1, 1, 8 * 1024, True, topk=4096, d_qk=576), [1, 2, 3, 4, 5, 10, 20, 40, 60, 80,100, 128]),
        (RawTestParam(0, 128, 1, 1, 16 * 1024, True, topk=4096, d_qk=576), [1, 2, 3, 4, 5, 10, 20, 40, 60, 80,100, 128]),
        (RawTestParam(0, 128, 1, 1, 32 * 1024, True, topk=4096, d_qk=576), [1, 2, 3, 4, 5, 10, 20, 40, 60, 80,100, 128]),
        (RawTestParam(0, 128, 1, 1, 64 * 1024, True, topk=4096, d_qk=576), [1, 2, 3, 4, 5, 10, 20, 40, 60, 80,100]),
        (RawTestParam(0, 128, 1, 1, 128 * 1024, True, topk=4096, d_qk=576), [1, 2, 3, 4, 5, 10, 20, 40]),
        ...
        # 精度对比 
        # (RawTestParam(0, 128, 1, 1, 16 * 1024, True, topk=2048, d_qk=576), [4]),
        # (RawTestParam(0, 128, 1, 1, 32 * 1024, True, topk=2048, d_qk=576), [2]),
        # (RawTestParam(0, 128, 1, 1, 64 * 1024, True, topk=2048, d_qk=576), [1]),

        # (RawTestParam(0, 128, 1, 1, 16 * 1024, True, topk=4096, d_qk=576), [4]),
        # (RawTestParam(0, 128, 1, 1, 32 * 1024, True, topk=4096, d_qk=576), [2]),
        # (RawTestParam(0, 128, 1, 1, 64 * 1024, True, topk=4096, d_qk=576), [1]),
    ]
    performance_cases = [
        # Production cases
...
def test_flash_mla(p: TestParam) -> Result:
    ...
    # 从 Torch 生成的测试用例加载
    # compare_base_dir = "/root/paddlejob/share-storage/gpfs/system-public/lhd/mla/compare"
    # input_dir = os.path.join(compare_base_dir, "input")
    # paddle_result_dir = os.path.join(compare_base_dir, "output/paddle")
    # os.makedirs(paddle_result_dir, exist_ok=True)
    ...
    # d_qk = p.d_qk
    # topk = p.topk
    # filename = f"decoding_sparse_b{b}_s{s}_h{h_q}_d{d_qk}_topk{topk}.npz"
    # input_path = os.path.join(input_dir, filename)
    
    # 加载测试用例（注释掉原有的生成方式）
    # t = lib.TestcaseForDecode.load_from_npz(input_path, p)
    ...
    if p.check_correctness:
        ...
        # 保存 Paddle CUDA 执行结果
        # paddle_result_path = os.path.join(paddle_result_dir, filename)
        # np.savez(paddle_result_path,
        #          out_ans=out_ans.detach().cpu().float().numpy(),
        #          lse_ans=lse_ans.detach().cpu().numpy())
        # print(f"Saved Paddle result to {paddle_result_path}")
    ...
    if p.num_runs == 0:
        ...
        result = kk.bench_paddle(run_decode, p.num_runs)
