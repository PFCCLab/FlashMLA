...
def ref_sparse_attn_fwd(p: TestParam, t: Testcase) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    if t.topk_length is not None:
        mask = torch.arange(p.topk, dtype=torch.int32, device=t.topk_length.device).unsqueeze(0).broadcast_to(p.s_q, p.topk) >= t.topk_length.unsqueeze(1)   # [s_q, topk] [s_q, topk]opk]k]
        indices[mask] = -1
...
def ref_sparse_attn_decode(
    ...
    def process_kv_scope(kv_scope: KVScope) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        indices_in_kv_cache_fixed = torch.clip(kv_scope.indices_in_kvcache, 0) # Otherwise torch.index_select will complain
        ...
        if kv_scope.topk_length is not None:
            invalid_mask |= torch.arange(0, topk, dtype=torch.int32).view(1, 1, topk).broadcast_to(b, p.s_q, topk) >= kv_scope.topk_length.view(b, 1, 1)
