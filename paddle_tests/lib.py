...
class ExtraTestParamForPrefill:
    b: int
    seqlens: List[int]  # sequence length of each batch
...
class TestParam:
    ...
    prefill: Optional[ExtraTestParamForPrefill] = None

@dataclasses.dataclass
class RawTestParamForPrefill:
    """
    "Flattened" test parameters for prefill test
    """
    bsz: int
    seqlens: List[int]  # sequence length of each batch
    # s_kv: List[int]  # s_kv[i] equals to s_q[i]
    topk: int
    h_q: int = 128
    h_kv: int = 1
    d_qk: int = 512
    ...
    def to_test_param(self) -> TestParam:
        return TestParam(
            sum(self.seqlens), sum(self.seqlens), self.topk, self.h_q, self.h_kv, self.d_qk, self.d_v,
            ...
            prefill = ExtraTestParamForPrefill(self.bsz, self.seqlens)
        )
...
class Testcase:
    ...
    def dump_as_npz(self, path: str):
        """
        Save testcase to npz file.

        Note: TestParam is not saved.
        """
        save_content = {
            "q": self.q.detach().cpu().float().numpy(),
            "kv": self.kv.detach().cpu().float().numpy(),
    ...
    def load_from_npz(path: str, p: TestParam):
        """
        Load testcase from npz file.
        """
        with np.load(path) as data:
            return Testcase(
                p,
                torch.tensor(data["q"], dtype=torch.bfloat16),
                torch.tensor(data["kv"], dtype=torch.bfloat16),
                torch.tensor(data["indices"], dtype=torch.int32),
                data["sm_scale"].item(),
...
def _randperm_batch(batch_size: int, perm_range: torch.Tensor, perm_size: int, paddings: List[int], chunk_size: int = 2048) -> torch.Tensor:
    """
    Generate random permutations in batch
    The return tensor, denoted as `res`, has a shape of [batch_size, perm_size]. `perm_range[0, i] <= res[i, :] < perm_range[1, i]` holds.
    Values within each row are unique.
    If, for some `i`, `perm_range[i] < perm_size` holds, then `res[i, :]` contains values in `[perm_range[0, i], perm_range[1, i])` as many as possible, and the rest are filled with `paddings`.
    """
    if perm_range.dim() == 2:
        range_stt = perm_range[0, :]
        range_end = perm_range[1, :]
        range_size = (range_end - range_stt).clamp(min=0)
        have_offsets = True
    else:
        range_stt = torch.zeros_like(perm_range)
        range_end = perm_range
        range_size = perm_range
        have_offsets = False

    res = torch.empty((batch_size, perm_size), dtype=torch.int32)

    for head in range(0, batch_size, chunk_size):
        tail = min(head + chunk_size, batch_size)
        cur_bs = tail - head

        chunk_range_stt = range_stt[head:tail]
        chunk_range_end = range_end[head:tail]
        chunk_range_size = range_size[head:tail]

        perm_range_max = max(
            int(chunk_range_size.max().item()),
            perm_size
        )

        rand = torch.rand(cur_bs, perm_range_max, dtype=torch.float32)

        idx = torch.arange(0, perm_range_max, dtype=torch.int32).view(1, -1).expand(cur_bs, -1)
        rand[idx >= chunk_range_size.view(cur_bs, 1)] = float("-inf")

        chunk_res = rand.topk(perm_size, dim=-1, sorted=True).indices.to(torch.int32)

        if have_offsets:
            chunk_res += chunk_range_stt.view(cur_bs, 1).to(torch.int32)

        padding_mask = (
            (chunk_res < chunk_range_stt.view(cur_bs, 1)) |
            (chunk_res >= chunk_range_end.view(cur_bs, 1))
        )

        if len(paddings) == 1:
            chunk_res[padding_mask] = paddings[0]
        else:
            fillers = torch.tensor(
                paddings, dtype=torch.int32
            )[torch.randint(
                0, len(paddings),
                (padding_mask.sum(),),
            )]
            chunk_res[padding_mask] = fillers

        res[head:tail] = chunk_res

    return res

def generate_testcase_for_prefill(t: TestParam) -> Testcase:
    kk.set_random_seed(t.seed)
    assert t.prefill is not None
    assert t.prefill.b == len(t.prefill.seqlens)
    assert t.s_q == sum(t.prefill.seqlens)
    ...
    seqlens = torch.tensor(t.prefill.seqlens, dtype=torch.int32)
    seq_isols = torch.vstack([
        torch.cat([torch.zeros(1, dtype=torch.int32), seqlens.cumsum(0, dtype=torch.int32)[:-1]]).repeat_interleave(seqlens),
        torch.arange(1, seqlens.sum() + 1, dtype=torch.int32)
    ])

    indices = _randperm_batch(t.s_q, seq_isols, t.topk, [-1]).view(t.s_q, t.h_kv, t.topk)

    if t.is_all_indices_invalid:
        all_indices_invalid_mask = torch.randn(t.s_q, device='cpu') < -2
        indices[all_indices_invalid_mask[:, None, None].broadcast_to(indices.shape)] = -1
